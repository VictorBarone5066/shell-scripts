...
while ($i < $maxDepth)
	...
	foreach j ($dirControl)
		...
		if (((-f POSCAR) || (-f CONTCAR) || (-f "vasprun.xml")) && (-f ../USE12)) then

#do stuff here------------------------------------------------------------------------------

#Find energy...
set nrg=`grep "e_fr_energy" "vasprun.xml" | tail -1 | tr -d "<i name=\"e_fr_energy\">" | tr -d "</i>" | sed "s/"//g"`
...
#Create POSCAR from xml file
cp /storage/home/vkb5066/scripts/poscarFromXML/genFiles.sh .
cp /storage/home/vkb5066/scripts/poscarFromXML/genPOSCAR .
./genFiles.sh
./genPOSCAR POSCAR
rm genFiles.sh
rm genPOSCAR
